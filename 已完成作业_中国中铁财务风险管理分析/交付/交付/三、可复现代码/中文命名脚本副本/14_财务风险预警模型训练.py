...
def write_markdown(dataset: pd.DataFrame, metrics: list[dict[str, str]], importances: list[dict[str, str]], predictions: list[dict[str, str]]) -> None:
    ...
    lines = [
        "# 机器学习财务风险预警基线模型",
        "",
        "复现脚本：`scripts/train_financial_warning_model.py`",
        "",
        "## 研究口径",
        "",
        "- 该模型是小样本财务指标基线模型，使用同业上市建筑企业财务面板训练。",
    ...
    lines.extend(
        [
            ...
            "## 使用边界",
            "",
            "- 样本量较小，测试集仅 11 条，指标只说明样本内训练和评估流程可以复现。",
            "- 标签是规则构造结果，后续应结合评级调整、违约事件、审计意见或更多企业风险事件进行校准。",
            "- 模型现阶段只使用财务指标训练；文本和事件指标已为中国中铁个案准备，暂未扩展到所有同业公司。",
        ]
    )
